...
def cmd_close(
    ...
    if spec.status == SpecStatus.IMPLEMENTED:
        error(
            ...
            json_out,
            {"error": "already_implemented", "id": spec_id},
        )
    ...
    if reason not in CLOSE_REASONS:
        ...
        error(
            ...
            json_out,
            {"error": "invalid_reason", "valid": list(CLOSE_REASONS)},
        )
    ...
    if not resolved_note and not yes and not json_out:
        ...
        resolved_note = (
            questionary.text(f"Close note (why is this being {reason}?):", style=style).ask() or ""
        )
    ...
    if resolved_note:
        ...
        spec.gate_notes = (
            (spec.gate_notes.rstrip() + f"\n\n---\n{close_line}") if spec.gate_notes else close_line
        )

    save_spec(spec, root)
    append_log(
        root,
        f"✕ CLOSED `{spec.id}` **{spec.title}** ({reason}){' — ' + resolved_note if resolved_note else ''}",
    )
    ...
    if cfg.git_auto_commit:
        try:
            from ..integrations.git import git_add_specs, git_commit_spec
