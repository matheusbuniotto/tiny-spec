...
def cmd_list(
    ...
    if agent:
        specs = [s for s in specs if not s.agent or s.agent == agent]
    ...
    has_agents = any(s.agent for s in specs)
    if has_assignees:
        table.add_column("Assignee", style="cyan", width=14, no_wrap=True)
    if has_agents:
        table.add_column("Agent", style="bright_blue", width=14, no_wrap=True)
    ...
    for i, s in enumerate(specs):
        ...
        if has_agents:
            row_args.append(s.agent or "[dim]any[/dim]")
