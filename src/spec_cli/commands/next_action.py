...
def cmd_next(json_out: bool, root: Path) -> None:
    ...
    unblocked = [s for s in active if not open_blockers(s, specs)]
    top = (unblocked or active)[0]
    blockers = open_blockers(top, specs)
    ...
    if json_out:
        claimable = [
            s
            for s in active
            if s.status == SpecStatus.APPROVED and not s.assignee and not open_blockers(s, specs)
        ]
        claimable.sort(key=lambda s: -_age_days(s.updated_at))
        queue = [
            {"id": s.id, "title": s.title, "age_days": _age_days(s.updated_at)}
