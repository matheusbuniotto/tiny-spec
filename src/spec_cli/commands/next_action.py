...
_COMMANDS: dict[SpecStatus, str] = {
    SpecStatus.AT_GATE: 'spec gate {id}  then  spec pass {id} --note "..." or spec reject {id} --note "..."',
    SpecStatus.IN_PROGRESS: "spec context {id} --json",
    SpecStatus.APPROVED: "spec claim {id} --yes --json",
    SpecStatus.DRAFT: "spec show {id}  then  spec approve {id}",
}


def cmd_next(json_out: bool, root: Path, agent: str = "") -> None:
    root = find_root(root)
    specs = list_specs(root)
    active = [s for s in specs if s.status not in (SpecStatus.IMPLEMENTED, SpecStatus.CLOSED)]
    if agent:
        active = [
            s for s in active if not s.agent or s.agent == agent or s.status == SpecStatus.AT_GATE
        ]
    ...
    if json_out:
        claimable = [
            ...
            and not s.assignee
            and (not agent or not s.agent or s.agent == agent)
        ]
        claimable.sort(key=lambda s: -age_days(s.updated_at))
        queue = [
            {"id": s.id, "title": s.title, "age_days": age_days(s.updated_at)}
            for s in claimable[:3]
        ]
        typer.echo(
            json.dumps(
                {
                    ...
                    "agent": top.agent,
                    "actor": "human" if top.status == SpecStatus.AT_GATE else "agent",
                    "blocked_for_agent": top.status == SpecStatus.AT_GATE,
                    "action": action,
                    "command": cmd,
                    "age_days": days,
